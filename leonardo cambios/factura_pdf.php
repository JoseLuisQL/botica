...
$accion7=mysqli_query($con, "select * from detalle_factura where tipo_doc=$tipo and tienda=$tienda1 and ven_com=1 ORDER BY  `detalle_factura`.`numero_factura` DESC  LIMIT 0 , 1");
...
while ($row=mysqli_fetch_array($sql))
	...
	$insert_detail=mysqli_query($con, "INSERT INTO detalle_factura VALUES (NULL,'$id_cliente','$id_vendedor','$numero_factura','$des','$id_producto','$cantidad','$precio_venta_r','$c','$ddd','1','$date_added','$costo','$tipo','$d','$moneda1','$folio','$peso','$num_bus')");  
        //ACTUALIZAR STOCK PARA FACTURAS/BOLETA/NOTA DE CREDITO       
        if($id_producto>0){
            if($des==1){
                $productos1=mysqli_query($con, "UPDATE products SET $b=$b-$cantidad,precio_producto=$precio_venta_r WHERE id_producto=$id_producto");
            }
            if($des==2){
                $productos1=mysqli_query($con, "UPDATE products SET $b=$b+$cantidad,precio_producto=$precio_venta_r WHERE id_producto=$id_producto");
            }
...
if($condiciones==4){
    $deuda=$sumador_total;
    $fecha7=date("Y-m-d",strtotime($fecha1."+ $dias7 days"));
}else{
    $deuda=0;
}
...
$insert=mysqli_query($con,"INSERT INTO facturas VALUES (NULL,'$numero_factura','$date','0','$nro_doc','$id_cliente','0','$id_vendedor','$condiciones','$sumador_total','$deuda','$tipo','$c','1','$ddd','1','$moneda1','$des','$num_bus','0','2018-11-11','$dias','$folio','$des','','0','$motivo','$tip')");
...
//$valor34="INSERT INTO facturas VALUES (NULL,'$numero_factura','$date','0','$nro_doc','$id_cliente','0','$id_vendedor','$condiciones','$sumador_total','$deuda','$tipo','$c','1','$ddd','1','$moneda1','$des','','$num_bus','2018-11-11','$dias','$folio','$des','','0','$motivo','$tip')";
...
//DIRECCION DE PDF
//print"$valor34";
header("location:ver_facturas.php?id_factura=$id_factura&t=1");
?>
