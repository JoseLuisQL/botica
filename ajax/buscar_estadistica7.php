...
if($q4==">=1"){
 ...
 for($i=$f1;$i<=$f2;$i++){
     ...
     //if($q==1 or $q==2){
     if($i<$f2){
         $text=$text." SUM(IF(DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fec[$j]' and aceptado='planilla' and ven_com=5 and condiciones$q6  and activo=1 and facturas.tienda$tienda,total_venta,0)) AS '$fec[$j]',";
         $text=$text." SUM(IF(DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fec[$j]' and aceptado='planilla' and ven_com=6 and condiciones$q6  and activo=1 and facturas.tienda$tienda,total_venta,0)) AS '$fec1[$j]',";
