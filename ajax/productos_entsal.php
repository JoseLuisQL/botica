				...
				while ($row=mysqli_fetch_array($query)){
					...
					<tr style="background-color: #CEE3F6;color:black;">
						...
						<td class='col-xs-2'>
                                                ...
                                                <td class='col-xs-1'>
                                                    <div class="pull-right">
                                                        <select class="form-control" style="text-align:right"  id="lote_<?php echo $id_producto; ?>" >
                                                        <?php
                                                        $sql1="SELECT * FROM  lote where id_producto=$id_producto and tienda=$tienda and final>0 ORDER BY lote.fec_fab ASC";
                                                        $query1 = mysqli_query($con, $sql1);
                                                        while ($row1=mysqli_fetch_array($query1)){
                                                            $lote=$row1['lote'];
                                                            $id_lote=$row1['id_lote'];
                                                             $final=$row1['final'];
                                                         ?> 
                                                          <option value="<?php echo $id_lote;?>"><?php print"Lote: $lote  Stock:$final" ;?></option>
