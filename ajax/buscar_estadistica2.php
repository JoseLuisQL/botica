...
if($q4==">=1"){
 ...
 for($i=$f1;$i<=$f2;$i++){
     ...
     if($q==1 or $q==2){
     if($i<$f2){
         $text=$text." SUM(IF(DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fec[$j]' and ven_com=1 and estado_factura<=3 and activo=1 and facturas.tienda$tienda,total_venta-obs,0)) AS '$fec[$j]',";
         $text=$text." SUM(IF(DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fec[$j]' and ven_com=2 and estado_factura<=3 and activo=1 and facturas.tienda$tienda,total_venta-obs,0)) AS '$fec1[$j]',";
     ...
     if($q==3){
     if($i<$f2){
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta-mone,0)) AS '$fec[$j]',";
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=2 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta-mone,0)) AS '$fec1[$j]',";
     }else{
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta-mone,0)) AS '$fec[$j]',";
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=2 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta-mone,0)) AS '$fec1[$j]'";
     }
