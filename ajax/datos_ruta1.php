...
<?php
	...
	if($action == 'ajax'){
		...
		if ( $_GET['q'] != "")
		{
			$sWhere.= " and  (products.des2 like '%$q%') ORDER BY  `products`.`nombre_producto` ASC";
		}
                if ( $_GET['q1'] != "")
		{
			$sWhere.= " and  (products.nombre_producto like '%$q1%') ORDER BY  `products`.`nombre_producto` ASC";
		}
                //print"$sWhere";
