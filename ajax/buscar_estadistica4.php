...
if($q4==">=1"){
 ...
 for($i=$f1;$i<=$f2;$i++){
     ...
     if($i<$f2){
         ...
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_compra-mone,0)) AS '$fec1[$j]',";
     }else{
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta-mone,0)) AS '$fec[$j]',";
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_compra-mone,0)) AS '$fec1[$j]'";
     }
