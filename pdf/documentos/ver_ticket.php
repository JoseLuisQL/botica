...
ob_start();	
	...
	//require_once(dirname(__FILE__).'/../html2pdf.class.php');
        $sql_factura22=mysqli_query($con,"select * from datosempresa");
        $rw_factura22=mysqli_fetch_array($sql_factura22);
        $conversion=$rw_factura22['conversion'];
...
if($servicio==1){
$sql=mysqli_query($con, "select * from detalle_factura, facturas where  detalle_factura.folio='$folio' and detalle_factura.numero_factura=facturas.numero_factura and detalle_factura.numero_factura=$numero_factura1 and detalle_factura.tienda=$tienda1 and facturas.ven_com=detalle_factura.ven_com and detalle_factura.tipo_doc='".$tipo1."' and facturas.id_factura='".$id_factura."' and detalle_factura.id_cliente='".$id_cliente."'" );
}
$suma=0;
$codigo_producto="";
while ($row=mysqli_fetch_array($sql))
	...
	echo "<tr>";
        echo "<td>".intval($cantidad1)."</td>";
        echo "<td>".$nombre_producto."<br>".$t1."</td>";
    ...
    if($tipo==0){
      ...
      echo number_format(($sumador_total-$obs),2,'.','');
      $op_gravada=number_format(($sumador_total-$obs)/(1+iva),2,'.','');  
    }
    if($tipo==1){
      echo $op_gravada="0.00";  
    }
    ...
    if($tipo==0){
        ...
        echo "0.00";
        $sumaigv=number_format(iva*(($sumador_total-$obs)/(1+iva)),2,'.','');  
    }
    if($tipo==1){
        echo $sumaigv="0.00";  
    }
    ...
    <?php 
    if($tipo==1){
        echo $sumaigv=number_format(($sumador_total-$obs),2,'.',''); 
    }
    if($tipo==0){
        echo $sumaigv="0.00";  
    }
    ...
    <td align="right"><b><?php echo moneda;?>  <?php echo $subtotal=number_format(($sumador_total-$obs),2,'.','');  ?></b></td>
    </tr>
    <?php
    ...
    <tr>
        <td colspan="4">
            <?php
            $decimales = explode(".",number_format(($sumador_total-$obs),2));
            $entera=explode(".",($sumador_total-$obs));
    ...
    <?php
     if($estado<=3 and ($obs>0 or $motivo>0)){
       ?>
    <tr>
      <td colspan="4">================================================</td>
    </tr>
</table>
<table border="0" align="center" width="335px">
    <tr>
        <td colspan="4" align="center"><font size="2"><strong>MONEDERO DEL AHORRO </font></strong></td>
         
    </tr>
    <tr>
        <td colspan="4" align="center"><font><strong><BR><?php echo $rw_cliente['nombre_cliente'];?><BR><br></strong> </font></td>
     ...
     if($estado<=3 and $obs>0){
       ?>
    
    <tr>
         <td colspan="3" align="left">MONEDERO UTILIZADO </td>
         <td colspan="1" align="left"><strong><?php echo round($obs*$conversion,2);?> puntos dinero</strong></td>
    </tr>
    <?php
    }
    if($estado<=3 and $motivo>0){
       ?>
    <tr>
         <td colspan="3" align="left">EN ESTA COMPRA HAS GANADO </td>
         <td colspan="1" align="left"><strong><?php echo round($motivo*$conversion,2);?> puntos dinero</strong></td>
    </tr>
    <?php
    }
    if($acu>0){
       ?>
    <tr>
         <td colspan="3" align="left">TIENES ACUMULADOS </td>
         <td colspan="1" align="left"><strong><?php echo round($acu*$conversion,2);?> puntos dinero</strong></td>
    </tr>
   
    <tr>
        <td colspan="4" align="left"><font><strong><BR>Sigue usando tu monedero del ahorro <?php echo $nombre;?><BR></strong> </font></td>
    ...
    <td colspan="4" align="center"><br>Autorizado mediante Resolución de Intendencia N° 032-
            005 Representación impresa de la <?php echo $doc;?>.
            Consulte su documento electrónico en:<span style="font-size: 14px">http:sunat.com.pe</span>
    </td>
    </tr>
