...
if($tipo<>8 and $tipo<>5 and $tipo<>6){
...
$tipo5=$tipo;
if($tipo==5 and strlen($doc1)<=8){
  $tipo5=$tipo+4;  
}
if($tipo==6 and strlen($doc1)<=8){
   $tipo5=$tipo+4; 
}
...
$count_query   = mysqli_query($con, "SELECT sum(cantidad_tmp*precio_tmp) AS numrows FROM tmp where tmp.session_id='".$session_id."'");
$row8= mysqli_fetch_array($count_query);
$suma45= $row8['numrows'];
$por=round(100*(1-($suma45-$monedero)/$suma45),2);
$sumador_total45=0;
...
while ($row=mysqli_fetch_array($sql))
	...
	$sumador_total+=$precio_total_r;//Sumador
        ...
        if($id_producto>0){
            ...
            $valor2=$row3['valor2'];
            $valor3=$row3['valor3'];
            $mon3=$row3['mon']*$cantidad*$precio_venta;
            $und_pro1=$row3['xml_und'];
            
        } 
        $igv=iva;
        $mon11=$mon11+$mon3;
        //$sumador_total45=$sumador_total45+$cantidad*round(($precio_venta-$por*$precio_venta/100),2);
        $cantidad1[$nums]=$cantidad;
        $und_pro[$nums]=$und_pro1;
        $tipo_cantidad[$nums]=$medida;
        $precio_unitario[$nums]=round($precio_venta-$por*$precio_venta/100,2);
        $a=1+iva;
        $valor_unitario[$nums]=$precio_unitario[$nums]/$a;
        $total_producto[$nums]=$valor_unitario[$nums]*$cantidad;
        $total_igv[$nums]=round($igv*$total_producto[$nums],2);
        $producto[$nums]=$nombre_producto;
        $codigo[$nums]=$codigo_producto;
        
        //INSERTAR DETALLE FACTURA
	$insert_detail=mysqli_query($con, "INSERT INTO detalle_factura VALUES (NULL,'$id_cliente','$id_vendedor','$numero_factura','$des','$id_producto','$cantidad','$precio_venta_r','$c','$ddd','1','$date_added','$costo','$tipo','$d','$moneda1','$folio','$valor2','$valor3','$mon3','$tipo8','$tipo9')");
        //ACTUALIZAR STOCK PARA FACTURAS/BOLETA/NOTA DE CREDITO    
        $id_detalle=mysqli_insert_id($con);
        
        if($id_producto>0 and $tipo<>8){
            if($des==1){
                $cantidad7=$cantidad*$tipo9;
                $productos1=mysqli_query($con, "UPDATE products SET $b=$b-$cantidad7 WHERE id_producto=$id_producto");
                $sql3=mysqli_query($con, "select * from lote,products where lote.final>0 and products.id_producto=lote.id_producto and lote.id_producto='".$id_producto."' ORDER BY lote.fec_vto ASC");
                $cant1=$cantidad7;
                while ($row5=mysqli_fetch_array($sql3))
                {
                    ...
                    $id_lote=$row5["id_lote"];
                    $insert_detail=mysqli_query($con, "INSERT INTO lote1 VALUES (NULL,'$id_lote','$date_added','$id_detalle','$cant2','$precio_venta_r','$des')");
            ...
            if($des==2){
                $cantidad7=$cantidad*$tipo9;
                $productos1=mysqli_query($con, "UPDATE products SET $b=$b+$cantidad7 WHERE id_producto=$id_producto and pro_ser=1");
            }
...
if($condiciones==4){
    $deuda=$sumador_total;
    $fecha7=date("Y-m-d",strtotime($fecha1."+ $dias days"));
}else{
    $deuda=0;
}
...
if($tipo<=3){
    $motivo=$mon11;
    $nro_doc=round($users-$monedero+$motivo,2);
}
$t1=$_SESSION['tienda'];
$cuenta="";
$tienda="tienda".$c;
...
if($tipo<>8){
$deuda1=mysqli_query($con, "UPDATE clientes SET deuda=deuda+$deuda,users=users-$monedero+$motivo WHERE id_cliente=$id_cliente");

$fecha333='2018-11-11';
$sql3="select * from caja where tienda=$t1 and cierre=0 and usuario_inicio=$id_vendedor ORDER BY caja.id_caja DESC LIMIT 0 , 1";
...
$insert=mysqli_query($con,"INSERT INTO facturas VALUES (NULL,'$numero_factura','$date','0','$nro_doc','$id_cliente','0','$id_vendedor','$condiciones','$sumador_total','$deuda','$tipo','$c','1','$ddd','1','$moneda1','$des','$monedero','0','2018-11-11','$dias','$folio','$des','','0','$motivo','$tip','$turno')");
...
if($tip==0){
    ...
    $cabecera["TOTAL_GRAVADAS"]=round($sumador_total/(1+iva),2);
    $cabecera["TOTAL_IGV"]=round(($sumador_total/(1+iva))*$igv,2);
    $cabecera["SUB_TOTAL"]=round($sumador_total/(1+iva),2);
    $cabecera["TOTAL"]=round($sumador_total,2);
    $cabecera["TOTAL_DESCUENTO"]=0;
    $mto_igv=round(($sumador_total/(1+iva))*$igv,2);
}
if($tip==1){
    $cabecera["TOTAL_EXONERADAS"]=0;
    $cabecera["TOTAL_INAFECTA"]=0;
...
<Invoice xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ccts="urn:un:unece:uncefact:documentation:2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:qdt="urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2" xmlns:udt="urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2" xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2">
	...
	<cac:AccountingCustomerParty>
        ...
        $xmlCPE = $xmlCPE .
            '<cac:PaymentTerms>
                ...
                <cbc:Amount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $cabecera["TOTAL"] . '</cbc:Amount>
                </cac:PaymentTerms>
            <cac:PaymentTerms>
                <cbc:ID>FormaPago</cbc:ID>
                <cbc:PaymentMeansID>Cuota001</cbc:PaymentMeansID>
                <cbc:Amount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $cabecera["TOTAL"] . '</cbc:Amount>
                <cbc:PaymentDueDate>' . $fecha7 . '</cbc:PaymentDueDate>
            </cac:PaymentTerms>';
        ...
        //$tot77=$cabecera["TOTAL_IGV"]+$ICBPER;
        $xmlCPE = $xmlCPE .           
	'<cac:AllowanceCharge>
    ...
    for($i=1;$i<=$nums-1;$i++){    
        ...
        if($tip==0){
        $cabecera["IMPORTE_DET"]=round(($cantidad1[$i]*$precio_unitario[$i])/(1+iva),2);
        $cabecera["PRECIO_DET"]=round($precio_unitario[$i],2);
        $cabecera["PRECIO_TIPO_CODIGO"]="01";    
        $cabecera["IGV"]=round((iva*$cantidad1[$i]*$precio_unitario[$i])/(1+iva),2);
        ...
        $cabecera["PRECIO_SIN_IGV_DET"]=round($precio_unitario[$i]/(1+iva),5);  
        }
        if($tip==1){
        ...
        $cabecera["DESCRIPCION_DET"]=trim($producto[$i]);
        $cabecera["CODIGO_DET"]=trim($codigo[$i]);
        $cabecera["PRECIO_SIN_IGV_DET"]=round($precio_unitario[$i],5);  
        }
...
for($i=1;$i<=$nums-1;$i++){    
        ...
        if ($tip==0){
            $cabecera["IMPORTE_DET"]=round(($cantidad1[$i]*$precio_unitario[$i])/(1+iva),2);
            $cabecera["PRECIO_SIN_IGV_DET"]=round($precio_unitario[$i]/(1+iva),2);
        }
        if ($tip==1){
            $cabecera["IMPORTE_DET"]=round(($cantidad1[$i]*$precio_unitario[$i]),2);
...
<cbc:PriceAmount currencyID="'.$cabecera["COD_MONEDA"].'">'.$cabecera["PRECIO_DET"].'</cbc:PriceAmount>
        ...
        if ($tip==1){
        $cabecera["EXO"]=$cantidad1[$i]*$precio_unitario[$i];
        $xmlCPE = $xmlCPE .
        '<cac:TaxTotal>
            <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">0.00</cbc:TaxAmount>
		<cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' .$cabecera["EXO"] . '</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">0.00</cbc:TaxAmount>
                    <cac:TaxCategory>
