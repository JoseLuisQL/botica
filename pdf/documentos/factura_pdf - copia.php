...
while ($row=mysqli_fetch_array($sql))
	...
	$sumador_total+=$precio_total_r;//Sumador
        ...
        $a=1+iva;
        $valor_unitario[$nums]=round($precio_unitario[$nums]/$a,2);
...
if($tip==0){
    ...
    $total=round($sumador_total45,2);
    $cabecera["TOTAL_GRAVADAS"]=round($sumador_total45/(1+iva),2);
    $cabecera["TOTAL_IGV"]=round(($sumador_total45/(1+iva))*$igv,2);
    $cabecera["SUB_TOTAL"]=round($sumador_total45/(1+iva),2);
    $cabecera["TOTAL"]=round($sumador_total45,2);
    $cabecera["TOTAL_DESCUENTO"]=0;
    $mto_igv=round(($sumador_total45/(1+iva))*$igv,2);
}
if($tip==1){
    $cabecera["TOTAL_EXONERADAS"]=0;
    $cabecera["TOTAL_INAFECTA"]=0;
    ...
    $cabecera["SUB_TOTAL"]=round($sumador_total45,2);
    $cabecera["TOTAL"]=round($sumador_total45,2);
    $cabecera["TOTAL_DESCUENTO"]=0;
    $cabecera["TOTAL_EXONERADAS"]=round($sumador_total45,2);
    $mto_igv=0;
}
